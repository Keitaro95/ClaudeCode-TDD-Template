---
description: 'テスト駆動開発（TDD）のRed-Green-Refactorサイクルを使用してテストを生成・更新する際のガイドライン。対話でテスト対象を絞り込み、厳格なTDDプロセスを実行します。'
applyTo: 'tests/**/*.py, src/**/*.py, documents/**/*.md'
---

# TDD統合テスト指示書

この指示書は、テスト駆動開発（TDD）のRed-Green-Refactorサイクルを実施するためのガイドラインです。
対話でテスト対象を絞り込み、コンテキスト消費を最小化します。
...
**推奨**: 複数テストを連続実行する場合、3-5サイクルごとにコンテキストをクリアすることを推奨します。
