...
### TDD用skillで、subagent(red, green, blue)を制御。
...
テスト作成用skillとsubagentを立てることで独立したコンテキストでTDDを実行できます(Claude Codeのコンテキスト管理機能による)。
skill、subagentは独立したコンテキストで実行されます。
そのため
- 他の設計書を勝手に読み込んだり
- 実装ありきでテストコードを書くのを
防ぐことができます。
 
特に1. phase1: red code では
すでに計画している実装を見せず、失敗させるテストを書くことに集中します。
実コードありきでテストコードを書かないことが大事になります。
あくまで振る舞いだけを検証するテストコードを書くことになります。
